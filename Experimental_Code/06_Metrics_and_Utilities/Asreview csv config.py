...
This version reads dataset configurations from a CSV file, making it easy to:
- Add/remove datasets without modifying code
- Organize large collections of systematic reviews
- Share configurations between team members
"""

import pandas as pd
from asreview_complete import ASReviewComplete, SAFEConfig
import os
...
def run_asreview_on_dataset(texts, labels, record_ids, topic, dataset_info, run_number):
    """Run ASReview on a single dataset."""
    config = SAFEConfig(
    ...
    output_dir = f"{dataset_info['category']}/{topic}/{topic}_complete_run_{run_number}"
...
def main():
    ...
    if all_results:
        ...
        summary_path = 'results/certainty/multi_dataset_summary.csv'
        ...
        for category in summary_df['category'].unique():
            ...
            category_path = f'results/{category}_summary.csv'
